...
void print_formated_seq_out(const char *pStr, float *pOut, int seqLen, int outSize)
{
    ...
    for (i = 0; i < seqLen; i++)
    {
        printf("---------------Time %d-----------------\n", i);
        for (j = 0; j < outSize; j++)
            printf("%5.5f ", pOut[i*outSize+j]);
        printf("\n\n");
    }
}

// Sankar : Sep 11, modified to add filename prm
void valid_char_rnn(char *cfgfile, char *weightfile, char *filename, char *seed)
{
    char *base = basecfg(cfgfile);
    fprintf(stderr, "%s\n", base);
    ...
    // Sankar: we consider a sequence of length net->time_steps
    float *input = calloc(inputs * net->time_steps, sizeof(float));
    //printf("input size = %d\n", inputs);
    int i;
    // Sankar Sep 11: replaced random input with an inp from file
    /*for(i = 0; i < len; ++i){
        c = seed[i];
        input[(int)c] = 1;
        network_predict(net, input);
    ...
    {
        int size = fread(input, 1, inputs * net->time_steps * sizeof(float), fp);
        if (size != inputs * net->time_steps * sizeof(float))
	    printf("error incomplete file \n");
        fclose(fp);
    }
    //print_formated_seq_out("INPUT", input, net->time_steps, net->inputs);
    float *out = network_predict(net, input);
...
void run_char_rnn(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "valid")) valid_char_rnn(cfg, weights, filename, seed);

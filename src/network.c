...
#ifdef CUDNN
        if(net->layers[i].type == CONVOLUTIONAL || net->layers[i].type == DEPTHWISE_CONVOLUTIONAL){
            cudnn_convolutional_setup(net->layers + i, cudnn_fastest);
        }
//	else if (net->layers[i].type == DEPTHWISE_CONVOLUTIONAL) {
...
void visualize_network(network net)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        layer l = net.layers[i];
        if(l.type == CONVOLUTIONAL || l.type == DEPTHWISE_CONVOLUTIONAL){
...
void fuse_conv_batchnorm(network net)
{
    ...
    for (j = 0; j < net.n; ++j) {
        ...
        if (l->type == DEPTHWISE_CONVOLUTIONAL) {
            //printf(" Merges Depthwsie- Convolutional-%d and batch_norm \n", j);

...
void forward_network(network *netp)
{
 ...
 lstm_time = 0;
	...
	net.index = i;
	net.layers[i].activation_orig = net.layers[i].activation;
        if ((i < net.n-1) && (net.layers[i].type == CONVOLUTIONAL) && (net.layers[i+1].type == MAXPOOL) && (net.layers[i].activation == LEAKY)){
	    net.layers[i].activation = ACT_NONE;
	    net.layers[i+1].activation = LEAKY;
	    //printf("layer %d swap \n", i);
	}
	else if ((i < net.n-1) &&(net.layers[i+1].type == MAXPOOL)){
	    net.layers[i+1].activation = ACT_NONE;
	}
        ...
        //printf("layer %d : fill in %f seconds.\n", i, what_time_is_it_now()-time);
         time=what_time_is_it_now();
        l.forward(l, net);
        //printf("layer %d : FWD in %f seconds.\n", i, what_time_is_it_now()-time);
        net.input = l.output;
        /*if (i ==11)
	     print_formated_seq_out("mp3", l.output, 1, 26*4);*/
        if(l.truth) {
            net.truth = l.output;
        }
        net.layers[i].activation = net.layers[i].activation_orig;
//printf("TOTAL TIME :: SGEMM = %f :: IM2COL = %f :: BN = %f :: ACT = %f :: MAXPOOL = %f FILL = %f\n", sgemm_time, im2col_time, bn_time, act_time, maxpool_time, fill_time);   
 }
    
    tot_bn_time += bn_time;
    tot_im2col_time += im2col_time;
    tot_act_time += act_time;
    tot_sgemm_time += sgemm_time;
    tot_maxpool_time += maxpool_time;
    tot_fill_time += fill_time;
    tot_lstm_time += lstm_time;
...
void forward_network_gpu(network *netp)
{
    ...
    double time=what_time_is_it_now();
    cuda_push_array(net.input_gpu, net.input, net.inputs*net.batch/net.time_steps);
    if(net.truth){
        cuda_push_array(net.truth_gpu, net.truth, net.truths*net.batch/net.time_steps);
    }
    //printf("running on gpu\n");
    int i;
    for(i = 0; i < net.n; ++i){
        net.index = i;
        // Sankar : Taking in optimizations from the CPU portion of the code
	net.layers[i].activation_orig = net.layers[i].activation;
#ifdef ACT_OPT_GPU
        if ((i < net.n-1) && (net.layers[i].type == CONVOLUTIONAL) && (net.layers[i+1].type == MAXPOOL) && (net.layers[i].activation == LEAKY)){
	    net.layers[i].activation = ACT_NONE;
...
#else
	if (net.layers[i].type == MAXPOOL) net.layers[i].activation = ACT_NONE;
#endif
        layer l = net.layers[i];

	//printf("layer %d inputs = %d outputs = %d batch = %d\n", i, l.inputs, l.outputs, l.batch);
        if(l.delta_gpu){
    ...
    //printf("time taken to net fwd gpu = %f\n", what_time_is_it_now()-time);
    calc_network_cost(netp);
}

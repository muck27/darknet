...
__global__ void add_bias1_kernel(float *output, float *biases, int batch, int n, int size, float slope)
{
    int index = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (index >= n*size*batch) return;
    int i = index % size;
    ...
    float t = output[(k*n+j)*size + i] + biases[j];
    output[(k*n+j)*size + i] = (t > 0)? t : slope*t;
}
...
void add_bias1_gpu(float *output, float *biases, int batch, int n, int size, ACTIVATION a)
{
    int num = n*size*batch;
    if (a == LEAKY){
        add_bias1_kernel<<<cuda_gridsize(num), BLOCK>>>(output, biases, batch, n, size, 0.1);
        check_error(cudaPeekAtLastError());
    }
    else{
        add_bias_kernel<<<cuda_gridsize(num), BLOCK>>>(output, biases, batch, n, size);
...
__global__ void normalize1_kernel(int N, float *x, float *mean, float *variance, float *scale, float *bias, int batch, int filters, int spatial)
{
    int index = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (index >= N) return;
    int f = (index/spatial)%filters;
    
    x[index] = x[index]*scale[f] + bias[f];
}
__global__ void normalize2_kernel(int N, float *x, float *mean, float *variance, float *scale, float *bias, int batch, int filters, int spatial, float slope)
{
    int index = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (index >= N) return;
    int f = (index/spatial)%filters;
    
    float t = x[index]*scale[f] + bias[f];
    x[index] = (t > 0)?t:(slope*t);
}
...
extern "C" void normalize1_gpu(float *x, float *mean, float *variance, float *scale, float *bias, int batch, int filters, int spatial, ACTIVATION a)
{
    size_t N = batch*filters*spatial;
    if (a != LEAKY){
    normalize1_kernel<<<cuda_gridsize(N), BLOCK>>>(N, x, mean, variance, scale, bias, batch, filters, spatial);
    check_error(cudaPeekAtLastError());
    }
    else{
    //printf("norm 2 kernel\n");
    normalize2_kernel<<<cuda_gridsize(N), BLOCK>>>(N, x, mean, variance, scale, bias, batch, filters, spatial, 0.1);
    check_error(cudaPeekAtLastError());
    }
}

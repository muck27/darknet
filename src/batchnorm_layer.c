...
{
    int opt = 1;
    //printf("Entering here batch norm layer cpu \n");
	...
	normalize_cpu1(l.output, l.rolling_mean, l.rolling_variance, l.scales, l.biases, l.batch, l.out_c, l.out_h*l.out_w, l.activation);
    }
    if (net.train || !opt){
...
{
    // Sankar nov 22 : optimize the calculation by combining mean var with scale bias
    int f;
#ifdef BN_OPT_GPU
    //printf("push batch norm \n");
    for (f = 0; f < l.c; f++)
    {
	l.scales[f] = l.scales[f]/(sqrt(l.rolling_variance[f]) + .000001f);
	l.biases[f] = l.biases[f] - l.rolling_mean[f]*l.scales[f];
    }
#endif
   // printf("push batch norm \n");
    //if (l.use_bias)
...
void forward_batchnorm_layer_gpu(layer l, network net)
{
    ...
    if (net.train) {
        ...
        add_bias_gpu(l.output_gpu, l.biases_gpu, l.batch, l.out_c, l.out_w*l.out_h);
            ...
            normalize1_gpu(l.output_gpu, l.rolling_mean_gpu, l.rolling_variance_gpu, l.scales_gpu, l.biases_gpu, l.batch, l.out_c, l.out_h*l.out_w, l.activation);
	}
    }
